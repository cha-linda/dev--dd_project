...
let hero = Array.from(classesData).find(element => element.roleClass === heroClass);
console.log(hero);
...
let heroMainFeatures = Array.from(heroBuild).filter(element => heroBuild.indexOf(element) > (heroBuild.length - 1) - 3);
let featuresToAssign = basisFeatures.filter(element => !heroMainFeatures.includes(element));
let heroLeftStat = [...heroStat]

console.log(featuresToAssign);
console.log(heroLeftStat);

Array.from(featuresToAssign).forEach(element => Object.assign(hero, { [element]: randomAssign(heroLeftStat) }));
...
console.log(hero.constit);
console.log(hero.lifeDice);
console.log(mod);
...
function setRaceBonus() {
    ...
    switch (hero.race) {
        case "Humain":
            ...
            hero.sag += 1;
            hero.char += 1;
            console.log("humain = +1 all features");
            break;
        case "Nain":
            // +2 constit
            hero.constit += 2;
            console.log("nain = +2 constit");
            break;
        case "Demi-elfe":
            ...
            console.log("demi-elfe = +2 char & +1 on two other random feat")
            break;
        case "Demi-orc":
            // +2 force & +1 constit
            hero.force += 2;
            hero.constit += 1;
            console.log("demi-orc = +2 force & +1 constit");
            break;
        case "Drakéide":
            // +2 force & +1 char
            hero.force += 2;
            hero.char += 1;
            console.log("drakeid = +2 force & +1 char");
            break;
        case "Gnome":
            // +2 int  
            hero.int += 2;
            console.log("gnome = +2 int");
            break;
        case "Tieffelin":
            // +1 int & +2 char
            hero.char += 2;
            hero.int += 1;
            console.log("tieffelin = +1 int & +2 char");
            break;
        default:
            // +2 dex (Elfe & Halfelin)  
            hero.dex += 2
            console.log("elfe or halfelin = +2 dex");
            break;
    }
}
setRaceBonus();
